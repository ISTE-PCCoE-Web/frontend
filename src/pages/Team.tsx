...
const populateData=async(ans, v1, v2, v3, v4, v5, v6, v7,v8)=>{
    ...
    useEffect(() => {
        ...
        const inSessionStorage = sessionStorage.getItem("membersData");
        if (!inSessionStorage) {
            fetch("https://us-central1-iste-pccoe.cloudfunctions.net/api/getTeamData")
                .then((data) => data.json())
                .then((response) => {
                    console.log("here is the request for backend data",response);
                    ...
                    sessionStorage.setItem("membersData", JSON.stringify(response));
